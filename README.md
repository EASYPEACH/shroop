...
## ☂️ 슈룹 팀의 일하는 방법
슈룹 팀이 협업에 있어서 중요하게 생각하는 요소 3가지  
1. **브레인 스토밍**: 카트에 찍찍이를 붙인다처럼 터무니없는 아이디어도 비판하지 않고, 적극적으로 아이디어를 내며 다양한 측면에서 사용자의 입장을 고려해보려 했다.  
2. **시각화**: 수업에서 진행했던 것처럼 miro와 같은 디자인 협업 툴이나, 포스트잇을 활용해보려했다. 대화만 진행하지 않고 주제를 구조화하고 시각화하여 팀원끼리의 의사소통을 더 수월하게 해보자고 목표를 정했다.
3. **마감기한**: 사소한 것마저 태스크를 산정하여 시간을 정하고, 그 시간을 꼭 엄수하고자 했다.
